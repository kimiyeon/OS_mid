...
|------|------|------------|
| Planning | Analysis | 주제 분석 및 토론 구조 설계 |
| Pro | Argument | 찬성 논리 생성 |
| Con | Argument | 반대 논리 및 반박 |
| Judge | Evaluation | PRO / CON 결정 |
| Verification | Validation | 결과 형식 및 조건 검증 |
| Gate | Decision | PASS / FAIL 판단 |
| Logging | Tracking | 실행 결과 및 개선 기록 |
...
Iteration 1:
- Initial output
- Errors and inconsistencies occur

Iteration 2:
- Previous log is used as input
...
|---------|------|--------|
| Language | Python | 전체 시스템 구현 |
| Framework | DeepAgents | 멀티 에이전트 구조 |
| LLM API | OpenRouter | 다양한 모델 비교 |
| Data | Pandas | 결과 분석 |
| Environment | Docker | 실행 환경 통일 |
| Config | dotenv | API key 관리 |
